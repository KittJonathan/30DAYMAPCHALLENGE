...
subregion <- boundaries$osm_multipolygons |> 
  filter(name == "Loch Lomond and The Trossachs National Park")
...
loch_lomond <- water$osm_multipolygons |> 
  filter(name == "Loch Lomond")

sort(unique(water$osm_polygons$name))

ggplot() +
  geom_sf(data = subregion$geometry,
          inherit.aes = FALSE) +
  geom_sf(data = loch_lomond$geometry,
          fill = "blue")
